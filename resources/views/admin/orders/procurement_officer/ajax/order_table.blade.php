                ...
                @foreach($data as $key)
                    <tr class="">
                        ...
                        <td>
                            ...
                            @else
                                @foreach($key->supplier as $child)
                                    <a href="{{ route('users.supplier.details',['id'=>$child['name']->id]) }}">{{ $child['name']->name }},</a>
                                @endforeach
                            @endif
                        </td>
                        <td>
                        ...
                        <td>
                            ...
                            @if(!empty($key->expected_arrival_date))
                                <span class="badge bg-success pt-1" style="font-size: 10px;display: block">وصول متوقع : <span>{{ $key->expected_arrival_date }}</span></span>
                            @endif
                        </td>
                        <td>
                            @if (auth()->user()->user_role != 9)
